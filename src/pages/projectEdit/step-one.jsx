...
const StepOne = ({ nextStep, prevStep, currentStep, setType, setSubType, setIsRent, setIsSale, isRent, isSale }) => {
    ...
    const getfilteredProjects = async (param) => {
        try {
            ...
            setFilteredProjects(sortedData.length ? sortedData : []);
        } catch (err) {
            console.log(err);
        } finally {
            setLoading(false);
    ...
    const getBuilderName = async (param) => {
        const project = projects.find((name) => name.id == param);
    ...
    const handleProjectName = async (e) => {
        setProjectNameform((prev) => ({ ...prev, [e.target.name]: e.target.value }))
    }
    ...
    const handleSubmitAddProjectForm = async () => {
        if (validateAddProjectForm()) {
            let res = await masterClient.post('projectname', projectNameForm);
            try {
                setLoading(true);
                if (res?.data?.status) {
                    ...
                    setProjectFormErr({});
                    setShow(false);
                    await fetchData(() => masterClient.get('projectname'), getfilteredProjects)
    ...
    const handleSubmitBuilderForm = async () => {
        if (validateBuilders()) {
            try {
                ...
                console.log(error);
                if (error?.response?.data?.type === 'Validation error' && error?.response?.data?.data) {
                    setBuilderFormErr(error?.response?.data?.data);
                } else {
                    toastError(error?.response?.data?.message);
                }
            } finally {
    ...
    const getLatLongs = (data) => {
        setCordinates(data);
        setProjectNameform({ ...form, latitude: data.lat, longitude: data.lng });
    }
