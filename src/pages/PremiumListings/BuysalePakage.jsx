...
const BuysalePakage = () => {
  ...
  const getListingPackages = async () => {
    setLoading(true);
    try {
      const role = userRole === 'Builder' ? 'Builder,Exclusive Sales Partner' : userData.role_id === 'Exclusive Sales Partner' ? 'Builder,Exclusive Sales Partner' : userRole
  ...
  useEffect(() => {
    ...
    if (result.length > 0) {
      ...
      toastWarning("No Packages Found")
    }
  }, [search])
