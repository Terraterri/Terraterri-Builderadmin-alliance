...
const StepTwo = ({ nextStep, prevStep, type, subType }) => {
  ...
  const handleChange = (e, formIndex) => {
    ...
    if (e.target.dataset.id === 'unitDetailsData') {
      ...
      if (name === 'property_bhk_size_id') {
        const bhkValue = bhkSize.find((item) => item.id == value);
        if (bhkValue) {
          updatedUnitDetails = updatedUnitDetails.map((item, index) => {
            if (index === formIndex) {
              return {
                ...item,
                car_parkings: bhkValue.no_of_parkings ?? item.car_parkings,
                balconies: bhkValue.no_of_balconies ?? item.balconies,
                bathrooms: bhkValue.no_of_bathrooms ?? item.bathrooms
              };
            }
            return item;
          });
      ...
      // Calculate total_base_price when base_price changes
      if (name === 'base_price' && value) {
        const updatedItem = updatedUnitDetails[formIndex];
        let totalBasePrice = 0;
        const basePrice = Number(value);

        // Try to calculate total_base_price using available area fields
        if (updatedItem.super_built_up_area) {
          totalBasePrice = basePrice * Number(updatedItem.super_built_up_area);
          updatedUnitDetails[formIndex] = { ...updatedItem, total_base_price: totalBasePrice };
        } else if (updatedItem.carpet_area) {
          totalBasePrice = basePrice * Number(updatedItem.carpet_area);
          updatedUnitDetails[formIndex] = { ...updatedItem, total_base_price: totalBasePrice };
        } else if (updatedItem.plot_size) {
          totalBasePrice = basePrice * Number(updatedItem.plot_size);
          updatedUnitDetails[formIndex] = { ...updatedItem, total_base_price: totalBasePrice };
        }
      }

      // Recalculate estimated_total_price when total_base_price is updated
      const recalculatedDetails = calculateTotalEstimatePrice(updatedUnitDetails);

      setUnitDetails(recalculatedDetails);
      setForm((prevForm) => ({ ...prevForm, unitDetails: recalculatedDetails }));
    } else {
  ...
  const handleImage = async (e, index) => {
    ...
    if (resFromMiddleware.clientStatus) {
      const fileUrl = resFromMiddleware.data.original_file_url || resFromMiddleware.data.original_image_url;
      if (index !== undefined) {
        setForm((prev) => ({
          ...prev,
          unitDetails: prev.unitDetails.map((unit, i) =>
            i == index
              ? { ...unit, [e.target.name]: fileUrl }
              : unit
          )
        }));
        setUnitDetails(
  ...
  const isPDF = (fileUrl) => {
    ...
    return fileUrl.toLowerCase().includes('.pdf') || fileUrl.toLowerCase().endsWith('pdf');
  };

  // Helper function to render file preview (image or PDF)
  const renderFilePreview = (fileUrl) => {
    if (!fileUrl) return null;

    if (isPDF(fileUrl)) {
      return (
        <div className="file-preview-pdf" style={{ padding: '15px', border: '1px solid #ddd', borderRadius: '4px', marginBottom: '10px', display: 'inline-block' }}>
          <a href={fileUrl} target="_blank" rel="noopener noreferrer" className="btn btn-outline-secondary btn-sm" style={{ textDecoration: 'none' }}>
            <i className="fas fa-file-pdf" style={{ marginRight: '8px' }}></i> View PDF
          </a>
        </div>
      );
    }

    return <img src={fileUrl} width="150" height="80" alt="Document Preview" style={{ border: '1px solid #ddd', borderRadius: '4px', marginBottom: '10px' }} />;
  };
  ...
  const blurValidation = (e, i) => {
    ...
    if (e.target.name === 'property_max_size') {
      ...
      if (minSizeValue && Number(e.target.value) < Number(minSizeValue)) {
    ...
    if (e.target.name == 'plot_size') {
      const minSize = form.property_min_size ? Number(form.property_min_size) : null;
      const maxSize = form.property_max_size ? Number(form.property_max_size) : null;
      const inputValue = Number(e.target.value);

      // Only validate if both min and max sizes are set and are valid numbers
      if (minSize !== null && maxSize !== null && !isNaN(minSize) && !isNaN(maxSize)) {
    ...
    if (e.target.name == 'super_built_up_area') {
      ...
      if (
        minSize && maxSize &&
        !(inputValue >= minSize && inputValue <= maxSize) &&
        subType == '7'
      ) {
        unitErrors = {
          ...unitErrors,
        ...
        form.property_min_size && form.property_max_size &&
        !(
          inputValue >= Number(form.property_min_size) &&
          inputValue <= Number(form.property_max_size)
        )
      ) {
        unitErrors = {
          ...unitErrors,
        ...
        // Recalculate total_base_price when super_built_up_area changes
        if (form.unitDetails[i]?.base_price && form.property_size_representation_id == 9) {
          let basePrice = form.unitDetails[i].base_price;
          let totalBasePrice = Number(basePrice) * Number(e.target.value);
          const updatedDetails = form.unitDetails.map((unit, index) =>
            index === i ? { ...unit, total_base_price: totalBasePrice } : unit
          );
          const recalculatedDetails = calculateTotalEstimatePrice(updatedDetails);
          setForm((prevForm) => ({
            ...prevForm,
            unitDetails: recalculatedDetails
          }));
          setUnitDetails(recalculatedDetails);
        }
      }
    ...
    if (e.target.name == 'carpet_area') {
      const superBuiltUpArea = form.unitDetails[i]?.super_built_up_area ? Number(form.unitDetails[i].super_built_up_area) : 0;
      const carpetAreaValue = Number(e.target.value);

      if (superBuiltUpArea && carpetAreaValue > superBuiltUpArea) {
        unitErrors = {
          ...unitErrors,
        ...
        // Recalculate total_base_price when carpet_area changes (for carpet area based pricing)
        if (form.unitDetails[i]?.base_price && form.property_size_representation_id == 10) {
          let basePrice = form.unitDetails[i].base_price;
          let totalBasePrice = Number(basePrice) * Number(e.target.value);
          const updatedDetails = form.unitDetails.map((unit, index) =>
            index === i ? { ...unit, total_base_price: totalBasePrice } : unit
    ...
    if (e.target.name == 'base_price') {
      ...
      if (form.property_size_representation_id == 9) {
        ...
        totalBasePrice = Number(basePrice) * Number(form.unitDetails[i].super_built_up_area);
      } else if (form.property_size_representation_id == 10) {
        let basePrice = e.target.value;
        totalBasePrice = Number(basePrice) * Number(form.unitDetails[i].carpet_area);
      } else if (subType == "9") {
        let basePrice = e.target.value;
        totalBasePrice = Number(basePrice) * Number(form.unitDetails[i].plot_size);
      }
    ...
    // Apply unit-level errors
    if (Object.keys(unitErrors).length > 0) {
      setFormError((prevError) => {
        const unitDetailsErrors = Array.isArray(prevError.unitDetails) ? [...prevError.unitDetails] : [];
        unitDetailsErrors[i] = { ...unitDetailsErrors[i], ...unitErrors };
        return { ...prevError, unitDetails: unitDetailsErrors };
      });
    }

    // Apply top-level errors
  ...
  const handleSubmit = () => {
    if (validate()) {
      ...
      let unitErrorsShown = false;

      if (formError.unitDetails && Array.isArray(formError.unitDetails)) {
        formError.unitDetails.forEach((unitError, index) => {
          if (unitError) {
            toastError(`Please fill Unit - ${index + 1} details`);
            unitErrorsShown = true;
          }
        });
      }
    ...
    <div>
            ...
            <div className="col">
              {form?.approval_document_path === undefined ? (
                ...
                <div className="col-md-12 imgclass">
                  {renderFilePreview(form?.approval_document_path)}
                  <button
                    className="btn btn-danger removebtn"
                    onClick={() => setForm((prev) => ({
                      ...prev,
                      approval_document_path: undefined
                    }))}
                  >
                    Delete {isPDF(form?.approval_document_path) ? 'Document' : 'Image'}
                  </button>
                </div>
              )}
          ...
          <div className="row mb-3">
            ...
            <div className="col">
              {form?.real_estate_approval_document_path === undefined ? (
                ...
                <div className="col-md-12 imgclass">
                  {renderFilePreview(form?.real_estate_approval_document_path)}
                  <button
                    className="btn btn-danger removebtn"
                    onClick={() => setForm((prev) => ({
                      ...prev,
                      real_estate_approval_document_path: undefined
                    }))}
                  >Delete {isPDF(form?.real_estate_approval_document_path) ? 'Document' : 'Image'}</button>
                </div>
              )}
      ...
      <div className="card">
        ...
        <div className="card-body">
          ...
          <div className="row">
            <h1 className="card-title mb-3 unitclass">
              {subType == 13 ? "Mall Layout Plan" :
                subType == "15" ? "IT Park Layout Plan" : " Project Layout Plan"}
            </h1>
            ...
            <div className="col-4 mb-3">
              {form?.project_layout_document_path === undefined ? (
                ...
                <div className="col-md-12 imgclass">
                  {renderFilePreview(form?.project_layout_document_path)}
                  <button
                    className="btn btn-danger removebtn "
                    onClick={() => setForm((prev) => ({
                      ...prev,
                      project_layout_document_path: undefined
                    }))}
                  >Delete {isPDF(form?.project_layout_document_path) ? 'Document' : 'Image'}</button>
                </div>
              )}
          ...
          <div className="mb-3">
            ...
            <CKEditor
              ...
              onChange={(event, editor) => {
                ...
                setForm((prev) => ({ ...prev, project_description: data }));
      ...
      <div className="card">
        ...
        <div className="card-body">
          <Tabs>
            <TabList className="unit-details-tablist d-flex align-items-center flex-wrap">
              {Array.from({ length: unitClone }).map((_, i) => (
                <Tab key={i}>{`unit - ${i + 1}`}</Tab>
              ))}
              {/* {formState.listing_type_id == 1 && (
                <button type="button" className="add-unit-tab ms-auto" onClick={checkUnitAndClone}>
                  Add Unit
                </button>
              )} */}
            </TabList>
              ...
              <TabPanel key={i}>
                    ...
                    <div className=" row mb-3 ">
                      <div className="form-floating col-3">
                        <select
                          ...
                          onBlur={blurValidation}>
                          <option value="">Select BHK Size</option>
                          {bhkSize.map((item, index) => (
                            <option key={index} value={item.id}>
                              {item.name}
                            </option>
                    ...
                    <div className="col-3 mb-3">
                      <div className="form-floating">
                        ...
                        <label for="super-buildup-area" className="fw-normal">
                          Super Build Up Area{form.sizeRepresentation ? `(${form.sizeRepresentation})` : "(Sq ft)"}
                        </label>
                      </div>
                    ...
                    <div className="col-3 mb-3">
                      <div className="form-floating">
                        ...
                        <label for="carpet-area" className="fw-normal">
                          Carpet Area{form.sizeRepresentation ? `(${form.sizeRepresentation})` : "(Sq ft)"}
                        </label>
                      </div>
                      ...
                      <div className="col-3 mb-3">
                        <div className="form-floating">
                          <select
                            ...
                            {(() => {
                              const selectedBhk = bhkSize.find((item) => item.id == form.unitDetails[i]?.property_bhk_size_id);
                              const maxParkings = selectedBhk?.no_of_parkings ? Number(selectedBhk.no_of_parkings) : 0;
                              return [...Array(maxParkings)].map((_, idx) => (
                      ...
                      <div className="col-3 mb-3">
                        <div className="form-floating">
                          <select
                            ...
                            {(() => {
                              const selectedBhk = bhkSize.find((item) => item.id == form.unitDetails[i]?.property_bhk_size_id);
                              const maxBalconies = selectedBhk?.no_of_balconies ? Number(selectedBhk.no_of_balconies) : 0;
                              return [...Array(maxBalconies)].map((_, idx) => (
                                <option key={idx + 1} value={idx + 1}>
                                  {idx + 1}
                                </option>
                              ));
                      ...
                      <div className="col-3 mb-3">
                        <div className="form-floating">
                          <select
                            ...
                            {(() => {
                              const selectedBhk = bhkSize.find((item) => item.id == form.unitDetails[i]?.property_bhk_size_id);
                              const maxBathrooms = selectedBhk?.no_of_bathrooms ? Number(selectedBhk.no_of_bathrooms) : 0;
                              return [...Array(maxBathrooms)].map((_, idx) => (
                                <option key={idx + 1} value={idx + 1}>
                                  {idx + 1}
                                </option>
                              ));
                ...
                <div className="Subcard">
                  ...
                  {form.unitDetails[i]?.floor_plan_path === undefined ? (
                    ...
                    <div className="col-md-12 imgclass">
                      {renderFilePreview(form?.unitDetails?.[i]?.floor_plan_path)}
                      <button
                        className="btn btn-danger removebtn"
                        onClick={() =>
                          setUnitDetails((prev) => ({ ...prev, floor_plan_path: undefined }))}
                      >Delete {isPDF(form?.unitDetails?.[i]?.floor_plan_path) ? 'Document' : 'Image'}</button>
                    </div>
                  )}
                </div>
                ...
                {unitClone - 1 === i &&
                  <div className='d-flex justify-content-center'>
                    <button className='btn plus-btn' onClick={checkUnitAndClone}>+Add New Unit</button>
                  </div>
                }
              </TabPanel>
            ))}
